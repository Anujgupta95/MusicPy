...
from helper import str_to_date_obj
import os
...
def index():
    ...
    songs = Song.query.filter_by(flagged="False").order_by(Song.release_date).all()
    albums_dict = dict()
    for song in songs:
    ...
    # print(albums_dict) = {"album name" : [songs, song2 ,song3],
    #                       "album 2" : []}
...
def playlist():
    ...
    for playlist in playlist_dict.keys():
        play_count[playlist] = len(playlist_dict[playlist])

    print(play_count)

    return render_template('playlist.html', user=user, likes_list=likes_list, user_playlists_dict=user_playlists_dict, playlist_dict=playlist_dict, play_count=play_count)
